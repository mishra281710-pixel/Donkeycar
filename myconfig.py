...
SCAN_Y = 100
SCAN_HEIGHT = 20
# HSV range for LIGHT BROWN

COLOR_THRESHOLD_LOW  = (0, 50, 50)    # HSV dark yellow (opencv HSV hue value is 0..179, saturation and value are both >
COLOR_THRESHOLD_HIGH = (50, 255, 255)
 # cap brightness to avoid orange/yellow
...
THROTTLE_MAX = 0.4    # maximum throttle value the controller will produce
...
OVERLAY_IMAGE = True  # True to draw computer vision overlay on camera image in web ui
                      # NOTE: this does not affect what is saved to the data                                                                              is this the  code for line following or lane following in donkeycar
# #
...
# #   although it is discouraged to mix RPI_GPIO and PIGPIO.
 #
SERVO_HBRIDGE_3PIN = {
    "FWD_PIN": "RPI_GPIO.BCM.23",   # ttl pin, high enables motor forward
    "BWD_PIN": "RPI_GPIO.BCM.24",   # ttl pin, high enables motor reverse
    "DUTY_PIN": "RPI_GPIO.BCM.18",  # provides duty cycle to motor
    "PWM_STEERING_PIN": "PCA9685.1:40.0",   # provides servo pulse to steering servo
    "PWM_STEERING_SCALE": 1.0,        # used to compensate for PWM frequency differents from 60hz; NOT for adjusting steering range
    "PWM_STEERING_INVERTED": False,   # True if hardware requires an inverted PWM pulse
    "STEERING_LEFT_PWM": 300,         # pwm value for full left steering (use `donkey calibrate` to measure value for your car)
    "STEERING_RIGHT_PWM": 600,        # pwm value for full right steering (use `donkey calibrate` to measure value for your car)
}
# 
# #
